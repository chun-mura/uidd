...
4. **実装**: 対象プロジェクトのフレームワークに合わせてコンポーネント + ストーリー (variant ごと) を実装する。案件で確定済みの UI 判断ルールが明示的に提供されている場合は、それを `uiux-principles` より優先する。原則ベースの判断が必要な場合は `${CLAUDE_PLUGIN_ROOT}/skills/uiux-principles/SKILL.md` の契約と WCAG 要点を参照し、Nielsen ヒューリスティクスまたはゲシュタルト原則が必要な場合のみ、対応する `${CLAUDE_PLUGIN_ROOT}/skills/uiux-principles/references/nielsen.md` または `${CLAUDE_PLUGIN_ROOT}/skills/uiux-principles/references/gestalt.md` を読む
