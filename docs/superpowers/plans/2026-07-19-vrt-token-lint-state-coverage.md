...
1. **baseline 確認**: 対象ストーリーごとに `docs/vrt/baselines/<story-id>.png` (対象プロジェクトのルートからの相対パス) の有無を確認する。`<story-id>` は Storybook の正準 ID (title を kebab-case 化し `--` で export 名を繋いだもの。例: `components-button--primary`) を使う。導出方法をセッションごとに変えない (変えると既存 baseline と一致せず、全件が「初回作成」扱いになり比較が空振りする)
...
ツールオーケストレーションモードでは該当するセクションのみ埋める (件数・レビュー URL は FAIL セクションに記載し、baseline 系セクションは「該当なし (ツールモード)」とする)。
````
...
4. Figma 乖離 diff (MCP がありトークン定義もある場合のみ。未実施なら「Figma 差分: 未実施 (MCP なし)」または「Figma 差分: 未実施 (トークン定義なし)」と理由を明記)
````
