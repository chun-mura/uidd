...
| `sb-verify.md` | 対象ストーリー (省略時全件) → チェック別 PASS/FAIL ブロック | Storybook 検証のオーケストレーター: ストーリー網羅 / a11y / インタラクション / 状態網羅の4チェックを独立実行・独立報告 |
...
| ファイル | 発火条件 |
|---------|------|
| `uiux-principles/` | WCAG の数値・達成基準を伴う a11y 設計判断の前。Nielsen ヒューリスティクスとゲシュタルト原則は、コマンドが必要時にのみ references から明示読込する (⚠️ 実案件での動作確認前) |
